...
RSpec.describe CglaCaseChartAssister::IllinoisEligibilityFlow do
  ...
  describe '#populate_eligibility' do
    ...
    before do
      ...
      allow(CglaCaseChartAssister::EligibilityUpdater).to receive(:apply_expungement_eligibility)
      allow(CglaCaseChartAssister::EligibilityUpdater).to receive(:apply_undetermined_eligibility)
      allow(CglaCaseChartAssister::EligibilityUpdater).to receive(:apply_sealable_eligibility)
      allow(CglaCaseChartAssister::EligibilityUpdater).to receive(:apply_disqualified_eligibility)
    end
    ...
    context 'when all events in the case are eligible for expungement' do
      ...
      it 'sets expungement info on the row' do
        expect(CglaCaseChartAssister::EligibilityUpdater).to receive(:apply_expungement_eligibility)
        subject.populate_eligibility(row, event, history)
      end
    end
    ...
    context 'when at least one event is not eligible for expungement and sealing eligibility cannot be determined' do
      ...
      it 'sets undetermined info on row' do
        expect(CglaCaseChartAssister::EligibilityUpdater).to receive(:apply_undetermined_eligibility)
        subject.populate_eligibility(row, event, history)
      end
    end
    ...
    context 'when at least one event is not eligible for expungement but all events in the case are eligible for sealing' do
      ...
      it 'sets sealing info on the row' do
        expect(CglaCaseChartAssister::EligibilityUpdater).to receive(:apply_sealable_eligibility)
        subject.populate_eligibility(row, event, history)
      end
    end
    ...
    context 'when at least one event is not eligible for sealing and all events were analyzed' do
      ...
      it 'sets disqualified info on the row' do
        expect(CglaCaseChartAssister::EligibilityUpdater).to receive(:apply_disqualified_eligibility)
        subject.populate_eligibility(row, event, history)
      end
    end
  end
