  ...
  class ChampaignCountyParser
    class << self
      ...
      def parse_court_cases(events)
        charges_by_case_number = group_by_case_number(events)
        charges_by_case_number.map do |case_number, charges_for_case|
          CourtCase.new(case_number: case_number, charges: charges_for_case)
        end
      end
